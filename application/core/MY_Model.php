...
class MY_Model extends CI_Model {
	...
	function update_detail($col,$id, $data)
	{
		if (!$id)
		{
			return FALSE;
	...
	*
	*/
	function get_info_rela($id, $field = '',$table='')
	{
		if (!$id)
		{
			return FALSE;
	 	...
	 	return $this->get_info_rule_rela($where, $field,$table);
	}

	/*
	*
	*/
	function get_info_rule_rela($where = array(), $field= '',$table='')
	{
	    if($field)
	    {
	        $this->db->select($field);
	...
	function get_total_query($table,$input = array())
	{
	...
	function get_relative($select,$table,$input = array())
	{
	    //xu ly ca du lieu dau vao
		$this->get_list_set_input($input);
